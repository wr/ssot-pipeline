...
#   - The reviewer GitHub App (matching review_bot_login in config/pipeline.json)
#     must be installed on the target repo before the first pr-review run.
...
from $SSOT_REPO. Adds Source of truth block to CLAUDE.md mapping
this repo to Linear project $PROJECT_NAME ($LINEAR_PROJECT_ID).

Generated by $SSOT_REPO/bin/init-target-repo.sh."
...
Generated by bin/init-target-repo.sh."
    ...
    echo "→ Pushed project_to_repo mapping to $SSOT_REPO (deploy-worker will auto-redeploy)"
  )
else
...
Done:
  ...
  • Pushed project_to_repo mapping to $SSOT_REPO (main) — deploy-worker is redeploying now
